...
### テストに関する考慮事項
- 依存性のモックを可能にするトレイトベースのインターフェース
- 明確な関心事の分離により分離テストが可能に
- リポジトリパターンはサービス層テストでのテストダブルを可能に
...
### 単体テスト
- コードとともに `mod tests` ブロックで単体テストを記述
- 成功ケースとエラー条件の両方をテスト
- 期待される動作を説明する記述的なテスト名を使用
- 必要に応じて依存性をモック

### 統合テスト
- `tests/` ディレクトリに統合テストを配置
- 複数コンポーネント間の相互作用をテスト
- アプリケーションコードと同じパターンを使用
